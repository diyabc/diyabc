...
void remplimatriceYP(int nli, int nco, int nmodel, long double** cmatP,
    ...
    for (i = 0; i < nli; i++) {
        ...
        for (imod = 0; imod < nmodel; imod++) {
            betax[imod] = 0.0;
            for (j = 0; j <= nco; j++) betax[imod] += cbeta[imod * (nco + 1) + j] * cmatX[i][j];
        }
    ...
    if (ma - mi < 1000.0) {
        ...
        for (imod = 1; imod < nmodel + 1; imod++) {
            ...
            for (j = 1; j < nmodel + 1; j++) {
                if (imod == j) vecD[j - 1] = esd * (sum - ebma); else vecD[j - 1] = -esd * ebma;
            }
            //cout<<"esd="<<esd<<"\n";
            sd[imod] = 0.0;
            for (j = 0; j < nmodel; j++) {
...
void calcul_psd(int nmodel, long double* b0, long double** matV,
    ...
    sum = 0.0;
    for (imod = 0; imod < nmodel + 1; imod++) sum += exp_safe(b0[imod] - ma);
    ...
    for (imod = 1; imod < nmodel + 1; imod++) {
        ...
        sd[imod] = 0.0;
        for (j = 0; j < nmodel; j++) for (l = 0; l < nmodel; l++) sd[imod] += vecD[j] * matV[j][l] * vecD[l];
    ...
    for (j = 0; j < nmodel; j++) vecD[j] = - exp_safe(b0[j + 1] - ma) / (sum * sum);
    sd[0] = 0.0;
...
inline int cherche_max(long double* vecY, int sizeY) {
    ...
    for (int i = 1; i < sizeY; i++) {
        if (vecY[i] > (long double)maxi) maxi = vecY[i] + .1;// transforme les .99999 en (int)1
    }
    return maxi;
}
...
int polytom_logistic_regression(int nli, int nco, long double** cmatX0,
    ...
    for (int i = 0; i < nli; i++) {
        csmatY[i] = 0.0;
        for (int j = 0; j < nmodel; j++) csmatY[i] += cmatY[i][j];
    }
    for (int i = 0; i < nli; i++) for (int j = 0; j < nco + 1; j++) cmatXT[j][i] = cmatX[i][j]; //transposition
    ...
    while (fin == false) {
        ...
        for (int i = 0; i < nmodnco; i++) for (int j = 0; j < nmodnco; j++) cmatC[i][j] = 0.0;
...
#pragma omp parallel for schedule(dynamic), shared(cmatC,cvecW,cmatP,cmatX)
        ...
        do {
            ...
            for (int i = 0; i < nmodnco; i++) for (int j = 0; j < nmodnco; j++) cmatB0[i][j] = cmatB[i][j];
        ...
        for (int i = 0; i < nmodnco; i++) for (int j = 0; j < nmodnco; j++) cmatB[i][j] = cmatB0[i][j];
        ...
        for (int i = 0; i < nmodnco; i++) {
            cdeltabeta[i] = 0.0;
            for (int j = 0; j < nmodnco; j++) cdeltabeta[i] += cmatB[i][j] * cmatYP[j]; // PP: bizarre avant rempli YP
        }
        for (int i = 0; i < nmodnco; i++)
        ...
        for (int i = 0; i < nmodel; i++) for (int j = 0; j < nmodel; j++) cmatA[i][j] = cmatB[i * (nco + 1)][j * (nco + 1)];
        ...
        for (int i = 0; i < nmodel + 1; i++) if (betmin > cbet[i]) betmin = cbet[i];
        for (int i = 0; i < nmodel + 1; i++) if (betmax < cbet[i]) betmax = cbet[i];
