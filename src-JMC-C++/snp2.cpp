...
//----------------------------------------
	...
	file2<<"POOL POP_NAME:HAPLOID_SAMPLE_SIZE  POP1:"<<ngenes<<"\n";
	for (loc=0;loc<nloc;loc++) {
		freq0=0.0;for (int i=0;i<nind;i++) freq0 +=(double)locus[i][loc]/(double)ngenes;
		nreads=mw.poisson(100);
		nreads0=mw.binom(nreads,freq0);
		file2<<nreads0<<"  "<<nreads-nreads0<<"\n";
		if ((nreads0==0)or(nreads0==nreads)) n++;
	}
	file2.close();
	cout<<"\n\nn0="<<n<<"\n";
	return 0;
