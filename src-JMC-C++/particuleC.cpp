...
int ParticleC::cree_haplo(int loc) {
    ...
    if ((debuglevel==11)and(loc<10)) {cout<<"\nLocus "<<loc<<"\n";this->gt[loc].ecris();cout<<"\n";}
...
bool ParticleC::mrcreached(int loc) {
    ...
    if (nr1<nr-nr1) mrc=nr1; else mrc=nr-nr1;
    //cout<<"mrc="<<mrc<<"\n";
    return (mrc>=this->mrc);
}
    ...
    for (int loc=0;loc<datasim.nloc;loc++) {
        for(int samp=0;samp<datasim.nsample;samp++) {
            ...
            for(int j=0;j<this->locuslist[0].samplesize[samp];j++) {
                if (this->locuslist[loc].haplosnp[samp][j]>0) nph++; else nnh++;
            }
            ...
            npr=this->mw.binom(nt,rp);nnr=nt-npr;
            spr=IntToString(npr);if (spr.length()==1) spr =" "+spr;
            snr=IntToString(nnr);if (snr.length()==1) snr =" "+snr;
            sgp += "   "+spr+" "+snr;
        }
        sgp +="\n";
    }
