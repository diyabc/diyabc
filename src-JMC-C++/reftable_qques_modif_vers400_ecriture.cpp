...
void ReftableC::bintotxt() {
	...
	while (not fifo.eof()) {
		...
		//for (int i=0;i<this->nstat;i++) fprintf(f1," %12.12e",enr.stat[i]); // ancienne version
		for (int i=0;i<this->nstat;i++) fprintf(f1,"  %e  ",enr.stat[i]); // MOFIF POUR MEME FORMAT PARAM ET STATS 23/11/15
		fprintf(f1,"\n");
	}
	this->closefile();
	enr.param.clear();
