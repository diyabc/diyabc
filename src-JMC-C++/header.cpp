...
void HeaderC::assignloc(bool sim, int gr){
	...
	int nl;
	if (sim) nl=datasim.nloc; else nl=dataobs.nloc;
	//cout<<"nl="<<nl<<"\n";
	groupe[gr].nloc = 0;
	...
	for (int i=0;i<nl;i++) {
		if (sim) {
			if (datasim.locus[i].groupe==gr) {iloc++;groupe[gr].loc[iloc] = i;}
		} else {
...
int HeaderC::readHeaderLoci(ifstream & file){
	...
	if (dataobs.filetype==0){ //fichier GENEPOP
		...
		for (gr=1;gr<=this->ngroupes;gr++) {
			groupe[gr].type = 2;if (dataobs.filetype==2) groupe[gr].type=3;
...
int HeaderC::readHeadersimDebut(ifstream & file){
    ...
    if (ss[0].find("poolseq")!=string::npos) {datasim.filetype=2;cout<<"fichier POOLSEQ\n";}
...
int HeaderC::readHeadersimScenario(ifstream & file){
	...
	vector<string> ss;
	int nlscen;
	getline(file,s1);   cout<<"première ligne scénario :"<<s1<<"\n";     //ligne vide
	getline(file,s1);   cout<<"deuxième ligne scénario :"<<s1<<"\n";    // nombre de scenarios
...
int HeaderC::readHeadersimHistParam(std::ifstream & file){
	...
	cout<<s1<<"\n";
	splitwords(s1," ",ss); 
	this->nparamtot=getwordint(ss[2],1);cout<<"nparamtot="<<this->nparamtot<<"\n";
...
int HeaderC::readHeadersimGroupPrior(std::ifstream & file){
	...
	for (int gr=1;gr<=this->ngroupes;gr++){
		...
		if (ss[2]=="[M]") {
			...
			getline(file,s1);splitwords(s1," ",ss1); groupe[gr].Pmoy   = atof(ss1[1].c_str());//cout<<"dans header Pmoy="<<groupe[gr].Pmoy<<"\n";
			...
			getline(file,s1);splitwords(s1," ",ss1); groupe[gr].snimoy = atof(ss1[1].c_str());//cout<<"dans header snimoy="<<groupe[gr].snimoy<<"\n";
