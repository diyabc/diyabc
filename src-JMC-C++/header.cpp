...
void HeaderC::assignloc(bool sim, int gr)
{
    ...
    for (int loc = 0; loc < nl; loc++)
    {
        if (sim)
        {
            if (datasim.locus[loc].groupe == gr)
                groupe[gr].nloc++;
        }
        else
        {
            if (dataobs.locus[loc].groupe == gr)
                groupe[gr].nloc++;
        }
    }
    if (debuglevel == 2)
        cout << "assignloc groupe[" << gr << "].nloc=" << groupe[gr].nloc << "   nloc=" << nl << "\n";
...
int HeaderC::readHeaderDebut(ifstream &file)
{
    ...
    if (debuglevel == 1)
        cout << "lecture du fichier data terminée\n";
    getline(file, s1);
    nl++;
...
int HeaderC::readHeaderScenarios(ifstream &file)
{
    ...
    delete[] nlscen;
    if (debuglevel == 2)
        cout << "header.txt : fin de la lecture de la partie scénarios\n";
    return 0;
}
...
int HeaderC::readHeaderHistParam(ifstream &file)
{
    ...
    for (int i = 0; i < this->nparamtot; i++)
    {
        ...
        if (ss[1] == "N")
            this->histparam[i].category = 0;
        else if (ss[1] == "T")
            this->histparam[i].category = 1;
        else if (ss[1] == "A")
            this->histparam[i].category = 2;
    ...
    if (this->nconditions > 0)
    {
        ...
        getline(file, s1);
        if (s1 != "DRAW UNTIL")
            this->drawuntil = false;
        else
            this->drawuntil = true;
    }
    if (debuglevel == 2)
        cout << "header.txt : fin de la lecture de la partie priors des paramètres démographiques\n";
        ...
        for (int i = 0; i < this->nscenarios; i++)
        {
            ...
            for (j = 0; j < this->nconditions; j++)
            {
                ...
                for (k = 0; k < scenario[i].nparam; k++)
                {
                    ...
                    if (this->condition[j].param1 == scenario[i].histparam[k].name)
                        np++;
                    if (this->condition[j].param2 == scenario[i].histparam[k].name)
                        np++;
                }
                if (np == 2)
                    nc++;
...
int HeaderC::readHeaderLoci(ifstream &file)
{
    ...
    { //fichier SNP
        ...
        for (gr = 1; gr <= this->ngroupes; gr++)
        {
            ...
            cout << "prem = " << prem << "\n";
            for (k = 0; k < nsg; k++)
                groupe[gr].nloc += getwordint(s1, 2 * k + 1);
            cout << "groupe[" << gr << "].nloc=" << groupe[gr].nloc << "\n";
            groupe[0].nloc -= groupe[gr].nloc;
            if (debuglevel == 2)
                for (int kk = 0; kk <= this->ngroupes; kk++)
                    cout << "groupe[" << kk << "].nloc = " << groupe[kk].nloc << "\n";
            groupe[gr].loc = vector<int>(groupe[gr].nloc);
            k1 = 0;
            for (k = 0; k < nsg; k++)
            {
                ...
                {
                    if (dataobs.filetype == 1)
                        typ = 10;
                    else if (dataobs.filetype == 2)
                        typ = 15;
                }
                else if (ss[2 * k + 1].find("<H>") != string::npos)
                    typ = 11;
                else if (ss[2 * k + 1].find("<X>") != string::npos)
                    typ = 12;
                else if (ss[2 * k + 1].find("<Y>") != string::npos)
                    typ = 13;
                else if (ss[2 * k + 1].find("<M>") != string::npos)
                    typ = 14;
        ...
        pivlocus.resize(0);
        for (int i = 0; i < groupe[1].nloc; i++)
            pivlocus.push_back(dataobs.locus[groupe[1].loc[i]]);
        //cout<<"dataobs.locus[0].type="<<dataobs.locus[groupe[1].loc[0]].type<<"\n";
        //cout<<"pivlocus[0].type="<<pivlocus[0].type<<"\n";
        for (int loc = 0; loc < groupe[1].nloc; loc++)
            dataobs.locus[loc] = pivlocus[loc];
        //cout<<"dataobs.locus[0].type="<<dataobs.locus[0].type<<"\n";
        pivlocus.clear();
        //cout<<"dataobs.locus.size()="<<dataobs.locus.size()<<"\n";
        for (int loc = dataobs.nloc - 1; loc >= groupe[1].nloc; loc--)
            dataobs.locus.pop_back();
        cout << "dataobs.locus.size()=" << dataobs.locus.size() << "\n";
        dataobs.nloc = (int)dataobs.locus.size();
        for (int loc = 0; loc < dataobs.nloc; loc++)
            groupe[1].loc[loc] = loc;
    ...
    if (debuglevel == 2)
        cout << "header.txt : fin de la lecture de la partie description des locus\n";
    return 0;
}
...
int HeaderC::readHeaderGroupPrior(ifstream &file)
{
    ...
    if (dataobs.filetype == 0)
    {
        ...
        for (gr = 1; gr <= this->ngroupes; gr++)
        {
            ...
            if (ss[2] == "[M]")
            {
                ...
                if (groupe[gr].priormutmoy.constant)
                    groupe[gr].mutmoy = groupe[gr].priormutmoy.mini;
                else
                {
                ...
                if (groupe[gr].priorPmoy.constant)
                    groupe[gr].Pmoy = groupe[gr].priorPmoy.mini;
                else
                {
                    groupe[gr].Pmoy = -1.0;
                    for (int i = 0; i < this->nscenarios; i++)
                ...
                if (groupe[gr].priorsnimoy.constant)
                    groupe[gr].snimoy = groupe[gr].priorsnimoy.mini;
                else
                {
            ...
            else if (ss[2] == "[S]")
            {
                ...
                if (groupe[gr].priormusmoy.constant)
                    groupe[gr].musmoy = groupe[gr].priormusmoy.mini;
                else
                {
                ...
                if (groupe[gr].priork1moy.constant)
                    groupe[gr].k1moy = groupe[gr].priork1moy.mini;
                else
                    groupe[gr].k1moy = -1.0;
                ...
                if (groupe[gr].priork2moy.constant)
                    groupe[gr].k2moy = groupe[gr].priork2moy.mini;
                else
                    groupe[gr].k2moy = -1.0;
        ...
        if (debuglevel == 2)
            cout << "header.txt : fin de la lecture de la partie définition des groupes\n";
        //Mise à jour des locus séquences
        MwcGen mwc;
        int nsv;
        bool nouveau;
        if (debuglevel == 2)
            cout << "avant la boucle sur les locus\n";
        for (int loc = 0; loc < dataobs.nloc; loc++)
        {
            ...
            if ((dataobs.locus[loc].type > 4) and (dataobs.locus[loc].type < 10) and (gr > 0))
            {
                ...
                if (debuglevel == 2)
                    cout << "nsv = " << nsv << "\n";
                if (debuglevel == 2)
                    cout << "mutsit.size = " << dataobs.locus[loc].mutsit.size() << "\n";
                if (debuglevel == 2)
                    cout << "groupe[gr].gams = " << groupe[gr].gams << "\n";
                if (debuglevel == 2)
                    cout << "dataobs.locus[loc].dnalength = " << dataobs.locus[loc].dnalength << "\n";
                for (int i = 0; i < dataobs.locus[loc].dnalength; i++)
                {
                    dataobs.locus[loc].mutsit[i] = mwc.ggamma3(1.0, groupe[gr].gams);
                }
                if (debuglevel == 2)
                    cout << "apres tirage dans gamma3\n";
                int *sitefix;
                ...
                double s = 0.0;
                for (int i = 0; i < dataobs.locus[loc].dnalength; i++)
                    s += dataobs.locus[loc].mutsit[i];
                for (int i = 0; i < dataobs.locus[loc].dnalength; i++)
                    dataobs.locus[loc].mutsit[i] /= s;
            }
        }
        if (debuglevel == 2)
            cout << "header.txt : fin de la mise à jour des locus séquences\n";
        ...
        for (int i = 0; i < this->nscenarios; i++)
        {
            ...
            for (int j = 0; j < scenario[i].nparamvar; j++)
                scenario[i].paramvar[j] = -1.0;
            //scenario[i].ecris();
        }
...
int HeaderC::readHeaderAllStat(ifstream &file, string headerfilename)
{
    ...
    cout << "nsamp=" << nsamp << "    "
         << "nstat=" << this->nstat << "   nparamut=" << nparamut << "\n";
...
int HeaderC::readHeaderGroupStat(ifstream &file)
{
    ...
    if (debuglevel == 2)
        cout << "header.txt : fin de la lecture des summary stats\n";
    return 0;
}
...
int HeaderC::buildSuperScen()
{
    ...
    for (int i = 0; i < this->nscenarios; i++)
    {
        if (this->scen.nevent < scenario[i].nevent)
            this->scen.nevent = scenario[i].nevent;
    }
    this->scen.nn0 = 0;
    for (int i = 0; i < this->nscenarios; i++)
    {
        if (this->scen.nn0 < scenario[i].nn0)
            this->scen.nn0 = scenario[i].nn0;
    }
    this->scen.nsamp = 0;
    for (int i = 0; i < this->nscenarios; i++)
    {
        if (this->scen.nsamp < scenario[i].nsamp)
            this->scen.nsamp = scenario[i].nsamp;
    }
    this->scen.nparam = 0;
    for (int i = 0; i < this->nscenarios; i++)
    {
        if (this->scen.nparam < scenario[i].nparam)
            this->scen.nparam = scenario[i].nparam;
    }
    this->scen.nparamvar = 0;
    for (int i = 0; i < this->nscenarios; i++)
    {
        if (this->scen.nparamvar < scenario[i].nparamvar)
            this->scen.nparamvar = scenario[i].nparamvar;
    }
    this->scen.nconditions = 0;
    for (int i = 0; i < this->nscenarios; i++)
    {
        if (this->scen.nconditions < scenario[i].nconditions)
            this->scen.nconditions = scenario[i].nconditions;
    }
    ...
    if (this->scen.nconditions > 0)
        this->scen.condition = vector<ConditionC>(this->scen.nconditions);
    for (int i = 0; i < this->scen.nsamp; i++)
        this->scen.time_sample[i] = 0;
    for (int i = 0; i < this->scen.nsamp; i++)
        this->scen.stime_sample[i] = " ";
    for (int i = 0; i < this->scen.nparamvar + 3; i++)
        this->scen.paramvar[i] = -1;
    PriorC pr;
    pr.loi = "uniforme";
    pr.mini = 0.0;
    pr.maxi = 1.0;
    ...
    for (int i = 0; i < this->scen.nparam; i++)
        this->scen.histparam[i] = pp;
    //this->scen.ecris();
    //cout<<"apres superscen\n";
    if (debuglevel == 2)
        cout << "header.txt : fin de l'établissement du superscen\n";
...
int HeaderC::buildMutParam()
{
    ...
    if (debuglevel == 2)
        cout << "header.txt : fin de l'établissement des paramètres mutationnels   nparamut=" << this->nparamut << "\n";
    return 0;
}
...
int HeaderC::readHeaderEntete(ifstream &file)
{
    ...
    if (debuglevel == 2)
        cout << "nss=" << nss << "   nparamtot=" << nparamtot << "   nparamut=" << nparamut << "   nstat=" << nstat << "\n";
...
int HeaderC::readHeader(string headerfilename)
{
    ...
    file.clear();
    ifstream rfrf("headerRFRF.txt");
    if (rfrf.is_open())
    {
        rfrf.close();
        std::cout << "Renaming headerRFRF.txt to headerRF.txt" << std::endl;
        std::rename("headerRFRF.txt", "headerRF.txt");
    }
    return 0;
}
...
int HeaderC::readHeadersimDebut(ifstream &file)
{
    ...
    if (nss > 2)
        datasim.sexratio = atof(ss[2].c_str());
    else
        datasim.sexratio = 0.5;
    ...
    for (int i = 0; i < datasim.nsample; i++)
    {
        ...
        for (int j = 0; j < nf[i]; j++)
            datasim.indivsexe[i][j] = 2;
        for (int j = nf[i]; j < datasim.nind[i]; j++)
            datasim.indivsexe[i][j] = 1;
    }
...
int HeaderC::readHeadersimScenario(ifstream &file)
{
    ...
    if (debuglevel == 2)
        cout << "header.txt : fin de la lecture de la partie scénario(s)\n";
    return 0;
}
...
int HeaderC::readHeadersimHistParam(std::ifstream &file)
{
    ...
    for (int i = 0; i < scenario[0].nparam; i++)
        cout << scenario[0].histparam[i].name << "  (" << scenario[0].histparam[i].category << ")   " << scenario[0].histparam[i].value << "\n";
    if (debuglevel == 2)
        cout << "header.txt : fin de la lecture de la partie priors des paramètres démographiques\n";
...
int HeaderC::readHeadersimLoci(std::ifstream &file)
{
    ...
    for (int loc = 0; loc < datasim.nloc; loc++)
    {
        ...
        if (ss[1] == "<A>")
            datasim.locus[loc].type = 0;
        else if (ss[1] == "<H>")
            datasim.locus[loc].type = 1;
        else if (ss[1] == "<X>")
            datasim.locus[loc].type = 2;
        else if (ss[1] == "<Y>")
            datasim.locus[loc].type = 3;
        else if (ss[1] == "<M>")
            datasim.locus[loc].type = 4;
    ...
    for (int loc = 0; loc < datasim.nloc; loc++)
        datasim.locus[loc].nsample = datasim.nsample;
    datasim.catexist = vector<bool>(5);
    datasim.ssize.resize(5);
    for (int i = 0; i < 5; i++)
        datasim.catexist[i] = false;
                        ...
                        for (int ind = 0; ind < datasim.nind[sa]; ind++)
                        {
                            ...
                            case 3:
                                if (datasim.indivsexe[sa][ind] == 1)
                                {
                                    ...
                                    datasim.locus[loc].ploidie[sa][ind] = 0;
                                break;
                            case 4:
                                datasim.ssize[locustype][sa] += 1;
                                datasim.locus[loc].samplesize[sa] += 1;
...
int HeaderC::readHeadersimFin()
{
    ...
    if (datasim.filetype == 0)
    {
        for (int loc = 0; loc < datasim.nloc; loc++)
        {
            ...
            if ((gr > 0) and (datasim.locus[loc].type > 4))
            {
                ...
                for (int i = 0; i < datasim.locus[loc].dnalength; i++)
                    datasim.locus[loc].mutsit[i] = mwc.ggamma3(1.0, groupe[gr].gams);
                int *sitefix;
                ...
                double s = 0.0;
                for (int i = 0; i < datasim.locus[loc].dnalength; i++)
                    s += datasim.locus[loc].mutsit[i];
                for (int i = 0; i < datasim.locus[loc].dnalength; i++)
                    datasim.locus[loc].mutsit[i] /= s;
            }
        }
    }
    else
    ...
    //this->scen.ecris();
    if (debuglevel == 2)
        cout << "header.txt : apres la copie du scénario\n";
    return 0;
}
...
string HeaderC::calstatobs(string statobsfilename)
{
    ...
    for (int i = 0; i < ngr; i++)
        particuleobs.grouplist[i] = groupe[i];
    if (debuglevel == 2)
        cout << "Dans calstatobs nloc=" << groupe[1].nloc << "   type de groupe=" << particuleobs.grouplist[1].type << "\n";
    /*if (groupe[0].nloc>0){
	...
	*/
    ...
    if (debuglevel == 2)
        cout << "avant la partie locus  nloc=" << dataobs.nloc << "\n";
    particuleobs.locuslist = vector<LocusC>(dataobs.nloc);
    if (debuglevel == 2)
        cout << "après l'allocation mémoire des locus\n";
    for (int kloc = 0; kloc < dataobs.nloc; kloc++)
    {
        ...
        else if (dataobs.locus[kloc].type < 20)
        {
            ...
            for (int sa = 0; sa < particuleobs.nsample; sa++)
            {
                ...
                if (debuglevel == 2)
                    cout << "locus " << kloc << "     nreads1[" << sa << "]=" << particuleobs.locuslist[kloc].nreads1[sa] << "   nreads[" << sa << "]=" << particuleobs.locuslist[kloc].nreads[sa] << "\n";
            }
        }
        //cout<<"fin du locus "<<kloc<<"\n";
    }
    if (debuglevel == 2)
        cout << "apres la partie locus\n";
    //string *sb,
    string ent;
    if (debuglevel == 2)
        cout << "this->entetestat = " << this->entetestat << "\n";
    ...
    else
    {
        for (int j = 0; j < 5; j++)
            cout << "stat_obs[" << j << "]=" << this->stat_obs[j] << "\n";
        cout << "   .../...\n";
        for (int j = this->nstat - 6; j < this->nstat; j++)
            cout << "stat_obs[" << j << "]=" << this->stat_obs[j] << "\n";
