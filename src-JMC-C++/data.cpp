...
int DataC::readfilePoolSeq(string filename) {
    ...
    splitwords(s1, " ", ss);
    nss = ss.size();
    this->nsample = nss-2;cout<<"nsample="<<this->nsample<<"\n";
    popname.resize(this->nsample);
    sampsize.resize(this->nsample);
    for (int i=0;i<this->nsample;i++) {
        p=ss[i+2].find(":");
        popname[i]=ss[i+2].substr(0,p);cout<<"popname["<<i<<"]="<<popname[i];
        s=ss[i+2].substr(p+1,6);
        sampsize[i]=atoi(s.c_str());cout<<"   sampsize="<<sampsize[i]<<"\n";
    }
...
int DataC::loadfromfile(string filename) {
    ...
    if (this->filetype == 2) {
        ...
        this->purgelocMRCPOOLSEQ();cout<<"apres purge\n";
        for (loc=0;loc<this->nloc;loc++) this->cal_coeffcoal(loc);
        this->calcule_ss();
