...
void dosimfile(int seed) {
	...
	cout << "SIMFILE filetype=" << datasim.filetype << "\n";
	if (datasim.filetype == 0) cout << "nombre de fichiers genepop à simuler = " << ntest << "\n";
	if (datasim.filetype > 0) cout << "nombre de fichiers snp à simuler = " << ntest << "\n";
	...
	if (datasim.filetype == 2) {
		sgp = ps.simuldataSNPool(ntest, multithread, seed);
		cout << "simfile POOLSEQ\n";
	}
...
void dosimstat(int seed) {
	...
	while (not fp.eof()) {
		...
		if (ligne.length() > 5 && (ligne.find("scenario") == string::npos)) {
			...
			if (nparam[numscen[nsel0] - 1] == 0) {
				for (int j = 1; j < nss; j++) {
					if (ss[j] != "NA")nparam[numscen[nsel0] - 1]++;
				}
			}
			...
			for (int j = 1; j < nss; j++) {
				if (ss[j] != "NA") {
					phistar0[nsel0][j0] = (long double)atof(ss[j].c_str());
					j0++;
				}
			}
			/*if (nsel0==0){
